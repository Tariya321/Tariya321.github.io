...
第八章节的算法实验采用文中简称Llama-3-1B/8B的BF16模型、Project Gutenberg和拼接WikiText-2。默认窗口1024、sink token 16；top-k按32K/64K/128K上下文设256/512/1024。阈值调优以相对dense PPL增幅不超过5%为目标。性能框架结合真实H100计算测量、双路Xeon的CXL传输/轮询代理测量、DRAMSim3与RTL综合；DReX是模拟的512GB LPDDR5X设备，不是实际跑通的GPU+DReX成品。
